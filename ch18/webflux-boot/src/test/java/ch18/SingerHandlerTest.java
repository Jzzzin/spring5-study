...
public class SingerHandlerTest {
    ...
    public void printAllSingers() {
        ...
        Flux<Singer> singerFlux = exchange.exchange(request)
                .flatMapMany(response -> response.bodyToFlux(Singer.class));

        // collectList 는 flux 에서 데이터를 추출하는 용도
        Mono<List<Singer>> singerList = singerFlux.collectList();
